...
router.post('/notification', authMiddleware, teacherMiddleware, notificationValidator, async (req, res) => {
  try {
    ...
    res.status(200).json({users: await User.find({group: req.body.group})})
  } catch (e) {
    console.log(e)
  }
})
...
router.put('/delete_students', authMiddleware, teacherMiddleware, deleteStudentsValidators, async (req, res) => {
  try {
    ...
    const user = await User.updateMany({ _id: delete_students }, { $unset: { group: "" } })
    res.status(200).json({ group, user })
  } catch (e) {
    console.log(e)
  }
...
router.delete('/delete', authMiddleware, teacherMiddleware, groupDeleteValidator, async (req, res) => {
  try {
    ...
    res.status(200).json({users: await User.find({group: req.body.id}), deleteGroup})
  } catch (e) {
    console.log(e)
  }
})
