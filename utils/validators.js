...
exports.registerRules = [
  ...
  body('email', 'Incorrect e-mail format')
    ...
    .custom(async (value, { req }) => {
      ...
      if (user) throw new Error('User with current e-mail already exists')
      return true
    })
    .normalizeEmail(),
...
exports.groupNameRules = [
  body('name')
    ...
    .custom(async (value, { req }) => {
      ...
      if (group) throw new Error('Group with this name already exists')
      return true
    }),
...
exports.scheduleRules = [
  body('group')
    ...
    .custom(async (value, { req }) => {
      const group = await Group.findById(value)
      if (!group) throw new Error('Group does not exist')
      else if (await Schedule.findOne({ group })) throw new Error('Schedule for this group already exists')
      return true
    }),
